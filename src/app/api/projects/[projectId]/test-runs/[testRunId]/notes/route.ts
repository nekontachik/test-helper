  ...
  if (req.method === 'POST') {
    try {
      ...
      const note = await prisma.$executeRaw`
        INSERT INTO TestRunNote (id, content, testRunId, userId, createdAt, updatedAt)
        VALUES (${randomUUID()}, ${validated.content}, ${testRunId}, ${session.user.id}, ${new Date()}, ${new Date()})
      `;
      ...
      return NextResponse.json({
        ...
        testRunId: createdNote.testRunId,
